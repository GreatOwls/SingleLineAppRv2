...
import { storageService, type PersistedAppState } from './services/storageService';
...
const App: React.FC = () => {
  ...
  const saveStatusResetTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const finalizeSaveStatus = useCallback((status: SaveStatus) => {
    ...
    const delay = status === 'error' ? 4000 : 2000;
  ...
  useEffect(() => {
    ...
    const stateToPersist = history.present;

    autosaveTimeoutRef.current = window.setTimeout(async () => {
      startSaving();
      let nextStatus: SaveStatus = 'saved';
      try {
        await storageService.save(stateToPersist);
        setLastSavedAt(Date.now());
      } catch (error) {
        console.error('Autosave failed', error);
        nextStatus = 'error';
      } finally {
        finalizeSaveStatus(nextStatus);
      }
    }, AUTOSAVE_DELAY);
  ...
  }, [history.present, appMode, startSaving, finalizeSaveStatus]);
  ...
  const handleSaveDiagram = useCallback(async () => {
    ...
    let nextStatus: SaveStatus = 'saved';
    try {
      await storageService.save(history.present);
      setLastSavedAt(Date.now());
    } catch (error) {
  ...
  }, [history.present, startSaving, finalizeSaveStatus]);
  ...
  const handleLoadDiagram = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          if (candidate.diagramData && Array.isArray(candidate.diagramData.nodes) && Array.isArray(candidate.diagramData.links) && Array.isArray(candidate.componentTypes)) {
            nextState = candidate as AppState;
          } else if (Array.isArray((parsed as DiagramData).nodes) && Array.isArray((parsed as DiagramData).links)) {
            nextState = {
              diagramData: parsed as DiagramData,
              componentTypes: initialComponentTypes,
            };
          }
        }
        ...
        try {
          ...
          console.error('Failed to persist loaded diagram:', persistError);
          nextStatus = 'error';
          alert('Diagram loaded locally but failed to save to the server.');
        } finally {
          finalizeSaveStatus(nextStatus);
        }
      } catch (error) {
    ...
    event.target.value = '';
  }, [startSaving, finalizeSaveStatus]);
  ...
  const isEditingDisabled = isInitializing || appMode === 'view' || !!focusedGroupId || !!tracedNodeId;
